...
class AccountSettingsVC: BaseVC,CodeDelegate,UITextFieldDelegate {
    
    @IBOutlet weak var btnNoDriver: UIImageView!
...
//        didSet{
//            btnChangeLanguage.addTarget(self, action: #selector(didTapBtnChnageLanguage), for: .touchUpInside)
//        }
//    }
  //  @IBOutlet weak var changePasswordView:UIView!
    ...
    var selectedLanguage = Language.getLanguage()
...
//    }
   ...
   // @IBOutlet weak var lblChnageSelectLanguage:UILabel!
    ...
    @IBOutlet weak var lblNotifyWhenFundDepositToMyAccount:UILabel!
    @IBOutlet weak var lblNotifyMeWhenMyRedeemRequestIsAccpetedOrRejected:UILabel!
...
//        picker.dataSource = self
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        if textField == self.txtPanicContactNo {
            
            let aSet = NSCharacterSet(charactersIn:"0123456789").inverted
            let compSepByCharInSet = string.components(separatedBy: aSet)
            let numberFiltered:String = compSepByCharInSet.joined(separator: "")
            let totalText:String = describe(self.txtPanicContactNo.text)
...
//        btnChangeLanguage.setTitle(appConts.const.lBL_CHANGE_LANGUAGE.uppercased(), for: .normal)
        ...
        lblNotifyWhenFundDepositToMyAccount.text = appConts.const.nOTIFY_FUND
        lblNotifyMeWhenMyRedeemRequestIsAccpetedOrRejected.text = appConts.const.nOTIFY_REQUEST
        
        changeContactBtn.setTitle(appConts.const.CHANGE_CONTACT_NO.uppercased(), for: UIControlState.normal)
    }
    ...
    func layoutSetup(){
        ...
        //changePasswordView.applyBorder(color: .lightGray, width: 1.0)
    }
        ...
        if NetworkManager.isNetworkConneted() {
            ...
            let parameters: Parameters =
                ["userId":sharedAppDelegate().currentUser!.uId,
                 ...
                 "no_driver_found":settings.no_driver_found
                 ]
    ...
    func displaySettings(settings:AccountSetting){
        ...
        if settings.no_driver_found.lowercased() == "y" {
            self.btnNoDriver.image = #imageLiteral(resourceName: "checkbox_selected")
            
        }
        else{
            self.btnNoDriver.image = #imageLiteral(resourceName: "checkbox_deselected")
    ...
    func updatePaypalEmail(){
        ...
        if NetworkManager.isNetworkConneted() {
            ...
            WSManager.getResponseFrom(serviceUrl: URLConstants.Domains.ServiceUrl+URLConstants.Settings.editpaypalemail, parameters: parameters, successBlock: { (json, urlResponse) in
                ...
                self.sharedAppDelegate().currentUser?.paypalEmail = self.accountSetting.paypalEmail
    ...
    func fetchLanguageConst(){
        ...
        let alert = Alert()
        WSManager.getResponseFrom(serviceUrl: URLConstants.Domains.ServiceUrl+URLConstants.User.languageConst, parameters: ["deviceId":CustomerDefaults.getDeviceToken(),"lId":selectedLanguage.id,], successBlock: { (json, urlResponse) in
    ...
    // MARK: - Button Actions
    @objc func didTapBtnChnageLanguage(){
        if selectedLanguage.id == -1{
            self.alert(title: "", message: appConts.const.MSG_SELECT_LANGUAGE)
        }else{
    ...
    @IBAction func btnOpenCountryCodeClicked(_ sender: Any) {
        ...
        if self.countryCodes.count > 0 {
            ...
            let alert = Alert()
            alert.showAlert(titleStr: appConts.const.lBL_MESSAGE, messageStr: "Country code not available", buttonTitleStr: appConts.const.bTN_OK)
        }
    }
    ...
    @IBAction func btnChangePasswordClicked(_ sender: Any) {
        ...
        if (self.txtOldPassword.text?.isEmpty)!{
            ...
            alert.showAlert(titleStr: appConts.const.aLERT, messageStr: appConts.const.eNTER_NEW_PASSWORD, buttonTitleStr: appConts.const.bTN_OK)
        }
        else if (self.txtConfirmNewPassword.text?.isEmpty)!{
            alert.showAlert(titleStr: appConts.const.aLERT, messageStr: appConts.const.MSG_NEW_CONFIRM_PASSWORD, buttonTitleStr: appConts.const.bTN_OK)
        }
        else if (self.txtConfirmNewPassword.text != self.txtNewPassword.text){
            alert.showAlert(titleStr: appConts.const.aLERT, messageStr: appConts.const.MSG_PASSWORD_NOT_SAME, buttonTitleStr: appConts.const.bTN_OK)
        }
        else{
            self.changePassword()
    ...
    @IBAction func btnNoDriverClicked(_ sender: UIButton) {
        if self.accountSetting.no_driver_found == "y" {
            self.accountSetting.no_driver_found = "n"
        }
        else{
            self.accountSetting.no_driver_found = "y"
...
//        } else {
//            return languageItem[row - 1].languageName
//        }
//    }
//
...
//        } else{
//            if languageItem.count > 0{
//                txtLanguage.text = self.languageItem[row - 1].languageName
//                self.selectedLanguage = self.languageItem[row - 1]
//            }
//        }
//    }
//}
