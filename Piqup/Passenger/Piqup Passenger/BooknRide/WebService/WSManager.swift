...
class WSManager: NSObject {
    ...
    class func getResponseFrom(serviceUrl:String,successBlock:@escaping SuccessBlock,failureBlock:@escaping FailureBlock ) {
        
        let manager = Alamofire.SessionManager.default
        manager.session.configuration.timeoutIntervalForRequest = 60 //default 10
        manager.request(serviceUrl, method: .post, encoding: JSONEncoding.default, headers: [:]).responseJSON { response in
            switch response.result {
            case .success(let value):
                // handle success here
                print(value)
    ...
    class func getResponseFromDefaultEncoding(serviceUrl:String,parameters:Dictionary<String, Any>,successBlock:@escaping SuccessBlock,failureBlock:@escaping FailureBlock ) {
        
        let manager = Alamofire.SessionManager.default
        manager.session.configuration.timeoutIntervalForRequest = 60 //default 10
        manager.request(serviceUrl, method: .post, parameters: parameters, headers: [:]).responseJSON { response in
            switch response.result {
            case .success(let value):
                // handle success here
                print(value)
    ...
    class func getResponseFromMultiPart(serviceUrl:String,parameters:Dictionary<String, Any>,images:[String:Any]
        ...
        manager.upload(multipartFormData: { (multipartFormData:MultipartFormData)  in
            ...
            for (_,image) in images.enumerated(){
                ...
                let imageData = UIImageJPEGRepresentation(image.value as! UIImage, 0.6)
                
                multipartFormData.append(imageData!, withName: image.key, fileName: "\(image.key).png", mimeType: "image/png")
            }
