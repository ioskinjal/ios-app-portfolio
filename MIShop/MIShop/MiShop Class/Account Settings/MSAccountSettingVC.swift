...
class MSAccountSettingVC: BaseViewController {
    ...
    {
        super.viewDidLoad()
        setUpNavigation(vc: self, navigationTitle: "Account Settings", action: #selector(btnSideMenuOpen))
        self.setLnaguageContant()
    }
    func setLnaguageContant()
    {
        btnChangePassword .setTitle("Change Password", for: .normal)
        btnEmailNotification .setTitle("Email Notification", for: .normal)
        btnShippingAddress .setTitle("My Shipping Address", for: .normal)
    }
    ...
    {
        if segue.identifier == "MSAccountSettingPageVC"
        {
            if let imagesPageVC = segue.destination as? MSAccountSettingPageVC
            {
...
extension MSAccountSettingVC: MSAccountSettingPageVCDelegate
{
