...
class AttachmentHandler: NSObject{
    ...
    private enum AttachmentType: String{
        case camera, video, photoLibrary, captureVideo
    }
    
    enum AttachmentOption:String {
        case images, video, file
    }
    ...
    private func authorisationStatus(attachmentTypeEnum: AttachmentType, vc: UIViewController){
        ...
        switch status {
        case .authorized:
            if attachmentTypeEnum == AttachmentType.captureVideo{
                openCameraForCaptureVideo()
            }
        ...
        case .notDetermined:
            ...
            PHPhotoLibrary.requestAuthorization({ (status) in
                if status == PHAuthorizationStatus.authorized{
                    ...
                    print("access given")
                    if attachmentTypeEnum == AttachmentType.captureVideo{
                        self.openCameraForCaptureVideo()
                    }
                    if attachmentTypeEnum == AttachmentType.camera{
                        self.openCamera()
                    }
