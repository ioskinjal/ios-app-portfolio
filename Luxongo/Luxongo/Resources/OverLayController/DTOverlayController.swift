...
open class DTOverlayController: UIViewController, UIViewControllerTransitioningDelegate {
    ...
    open var _viewController: UIViewController
    ...
    override open func viewDidLoad() {
        ...
        updateCornerRadius()
        _viewController.view.layer.masksToBounds = true
        handle.layer.cornerRadius = handleSize.height / 2
        ...
        observeScrollViewUpdate(with: _viewController)
    }
    ...
    open override func viewWillLayoutSubviews() {
        ...
        _viewController.view.frame = CGRect(origin: CGPoint(x: 0, y: handleVerticalSpace), size: CGSize(width: size.width, height: size.height - handleVerticalSpace))
    }
    
    private func updateCornerRadius() {
        if #available(iOS 11.0, *) {
            _viewController.view.layer.maskedCorners = [.layerMinXMinYCorner, .layerMaxXMinYCorner]
            _viewController.view.layer.cornerRadius = overlayViewCornerRadius
        } else {
    ...
    private func addCornerRadiusMask() {
        let path = UIBezierPath(roundedRect: _viewController.view.bounds, byRoundingCorners: [.topLeft, .topRight], cornerRadii: CGSize(width: overlayViewCornerRadius, height: overlayViewCornerRadius))
        let mask = CAShapeLayer()
        mask.path = path.cgPath
        _viewController.view.layer.mask = mask
    }
