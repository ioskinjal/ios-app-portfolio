...
extension UIView {
    ...
    func border(side: BorderSide = .all, color:UIColor = UIColor.clear, borderWidth:CGFloat = 0.0) {
...
extension UIButton {
    func addTextSpacing(spacing: CGFloat,color:String, for text: String? = nil) {
        ...
        value: UIColor(hexString: color),
        range: NSRange(location: 0, length: text.count))
        self.setAttributedTitle(attributedString, for: .normal)
}
    func addTextSpacingButton(spacing: CGFloat){
...
extension UITextField{
    func addPlaceholderSpacing(spacing: CGFloat){
        ...
        attributedString.addAttribute(NSAttributedStringKey.kern, value: spacing, range: NSRange(location: 0, length: self.placeholder!.count))
...
extension UINavigationItem{
    func addSpacing(spacing: CGFloat){
        ...
        attributedString.addAttribute(NSAttributedStringKey.kern, value: spacing, range: NSRange(location: 0, length: self.title!.count))
        let label = UILabel()
...
extension UITextField{
    ...
    func placeHolder(text:String, textfieldname :UITextField)
     {
...
extension UITabBarController {
    ...
    func switchViewControllersTab(isArabic arabic : Bool){
        if arabic {
            UIView.appearance().semanticContentAttribute = .forceRightToLeft
...
extension UIImageView {
    ...
    @objc fileprivate func image(image: UIImage, didFinishSavingWithError error: NSError?, contextInfo:UnsafeRawPointer) {
        if error == nil {
            ...
            let ac = UIAlertController(title: "save_error".localized, message: error?.localizedDescription, preferredStyle: .alert)
            ac.addAction(UIAlertAction(title: "ok".localized, style: .default, handler: nil))
            self.viewController?.present(ac, animated: true, completion: nil)
...
extension UIImageView{
   // placeHolderImage img: UIImage = #imageLiteral(resourceName: "small-Image-Place-Holder")
    ...
    func downloadSdImage(url: String)
    {
        self.sd_setImage(with: URL(string: url), placeholderImage: #imageLiteral(resourceName: "placeholder"))
    }
    
    func downLoadImage(url: String, placeHolderImage img: UIImage = #imageLiteral(resourceName: "overlay_left")) {
...
extension Locale {
    static func getCurrentCountryName() -> String {
        var name = ""
        let locale = Locale.current//(identifier: "en_US")
        guard let countryCode = (locale as NSLocale).object(forKey: .countryCode) as? String else { return name }
        name = locale.countryName(from: countryCode)
        return name
    }
    
    func countryName(from countryCode: String) -> String {
        if let name = (self as NSLocale).displayName(forKey: .countryCode, value: countryCode) {
...
extension Double {
    var clean: String {
       ...
       // let formattedNumber = numberFormatter.string(from: NSNumber(value:self))
        return (self.truncatingRemainder(dividingBy: 1) == 0 ? numberFormatter.string(from: NSNumber(value:self)) : numberFormatter.string(from: NSNumber(value:self)))!
    }
}
extension UILabel {
