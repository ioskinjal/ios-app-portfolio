//
//  SearchRootClass.swift
//  Created on June 29, 2020

import Foundation


class SearchRootClass : NSObject, NSCoding{

    var shards : SearchShard!
    var hits : SearchHit!
    var timedOut : Bool!
    var took : Int!
    ...
    init(fromDictionary dictionary: [String:Any]){
        ...
        if let shardsData = dictionary["_shards"] as? [String:Any]{
            shards = SearchShard(fromDictionary: shardsData)
        }
        if let hitsData = dictionary["hits"] as? [String:Any]{
            hits = SearchHit(fromDictionary: hitsData)
        }
    }
    ...
    {
        shards = aDecoder.decodeObject(forKey: "_shards") as? SearchShard
        hits = aDecoder.decodeObject(forKey: "hits") as? SearchHit
