...
//  LevelShoes
//
//  Created by Ruslan Musagitov on 28.08.2020.
...
public class RMTextField: UITextField {
    ...
    public var dtBorderWidth:CGFloat                          = 0.5 {
        didSet {
        ...
        didSet{
            ...
            switch dtborderStyle {
                ...
                case .rounded:
                    ...
                    dtLayer.borderWidth         = dtBorderWidth
                ...
                case .bottom,.left,.right,.top:
                    dtLayer.cornerRadius        = 0.0
                    dtLayer.borderWidth         = 0.0
                    borderLayer.backgroundColor = dtBorderColor.cgColor
                    if dtborderStyle == .bottom {
                        borderLayer.frame = CGRect(x: 0, y: dtLayer.bounds.size.height - dtBorderWidth, width: dtLayer.bounds.size.width, height: dtBorderWidth)
                    }else if dtborderStyle == .left{
                        borderLayer.frame = CGRect(x: 0, y: 0, width: dtBorderWidth, height: dtLayer.bounds.size.height)
                    }else if dtborderStyle == .right{
                        borderLayer.frame = CGRect(x: dtLayer.bounds.size.width - dtBorderWidth, y: 0, width: dtBorderWidth, height: dtLayer.bounds.size.height)
                    }else{
                        borderLayer.frame = CGRect(x: 0, y: 0, width: dtLayer.bounds.size.width, height: dtBorderWidth)
                    }
    ...
    public var paddingYFloatLabel:CGFloat = 20.0 {
    ...
    fileprivate func commonInit() {
        ...
        floatPlaceholderColor       = colorNames.mobileTF_PlaceHolder_Active
        floatPlaceholderActiveColor = colorNames.mobileTF_PlaceHolder_Active
        floatLabelFrame   = CGRect.zero
        ...
        lblError.font               = errorFont
        lblError.textColor          = UIColor.blue
    ...
    func setFloatLabelAlignment() {
        ...
        print("set float frame 2")
        floatLabelFrame = newFrame
    }
    ...
    var floatLabelFrame: CGRect {
        set {
            print("new y \(newValue.origin.y) h \(newValue.height)")
            lblFloatPlaceholder.frame = newValue
        }
        get {
            return lblFloatPlaceholder.frame
        }
    }
    ...
    fileprivate func showFloatingLabel(_ animated:Bool) {
        let animations:(()->()) = {
            ...
            print(self.paddingYFloatLabel)
            
            print("set float frame 3")
            self.floatLabelFrame = CGRect(x: self.floatLabelFrame.origin.x,
                                          y: self.lblFloatPlaceholder.font.lineHeight - self.floatLabelFrame.height - self.paddingYFloatLabel,
    ...
    fileprivate func insetRectForBounds(rect:CGRect) -> CGRect {
        ...
        guard let placeholderText = lblFloatPlaceholder.text,!placeholderText.isEmptyStr  else {
            f = insetRectForEmptyBounds(rect: rect)
            return f!
        }
        if floatingDisplayStatus == .never {
            ...
            if let text = text,text.isEmptyStr && floatingDisplayStatus == .defaults {
                ...
                f = CGRect(x: x, y: 0, width: rect.size.width - x - paddingX, height: rect.height)
            }
        }
        return f!
    }
    ...
    override public var intrinsicContentSize: CGSize{
        ...
        if showErrorLabel {
            ...
            let h = textFieldIntrinsicContentSize.height + paddingYFloatLabel + paddingYErrorLabel + lblFloatPlaceholder.bounds.size.height + lblError.bounds.size.height + paddingHeight
            return CGSize(width: textFieldIntrinsicContentSize.width,
                          height: h)
        } else {
            let h = textFieldIntrinsicContentSize.height + paddingYFloatLabel + lblFloatPlaceholder.bounds.size.height + paddingHeight
            return CGSize(width: textFieldIntrinsicContentSize.width,
                          height: h)
        }
    }
    ...
    override public func layoutSubviews() {
        ...
        print("set float frame 1")
        floatLabelFrame = CGRect(x: x, y: floatLabelFrame.origin.y,
