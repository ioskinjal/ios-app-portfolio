...
class MenuVC: BaseVC,UITableViewDelegate,UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(constReload), name: NSNotification.Name("ConstChnage"), object: nil)
    ...
    override func viewDidLayoutSubviews() {
        ...
        if #available(iOS 11, *) {
            ...
            self.topLayoutConstraint = self.bgView.topAnchor.constraint(equalTo: self.topLayoutGuide.bottomAnchor)
    ...
    func menuSetup(){
        ...
        if let dict = myDict {
            // Use your dict here
            let list = (dict.object(forKey: "list")as! NSArray).mutableCopy() as! NSMutableArray
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell?.itemImgView.image = UIImage(named: item.object(forKey: "image") as! String)
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        DispatchQueue.main.async {
            ...
            let item = self.menuItems[indexPath.row] as! NSDictionary
            let controllerName = (item.object(forKey: "name") as! String)
            ...
            case "Financial Information":
                if !(self.slideMenuController()?.mainViewController?.childViewControllers.first?.isKind(of: FinanceInformationVC.self))!{
                    
                    let walletController = FinanceInformationVC(nibName: "FinanceInformationVC", bundle: nil)
                    let nvc: UINavigationController = UINavigationController(rootViewController: walletController)
                    nvc.setNavigationBarHidden(true, animated: false)
                    nvc.setToolbarHidden(true, animated: false)
                    self.slideMenuController()?.changeMainViewController(nvc, close: true)
            ...
            case "Logout":
                ...
                if isOnline {
                    alert.showAlert(titleStr: "", messageStr: appConts.const.cANNOT_OFFLINE, buttonTitleStr: appConts.const.bTN_YES)
                }
                else{
                    alert.showAlertWithLeftAndRightCompletionHandler(titleStr: appConts.const.lOGOUT, messageStr: appConts.const.mSG_LOGOUT, leftButtonTitle: appConts.const.bTN_YES, rightButtonTitle: appConts.const.bTN_NO, leftCompletionBlock: {
                        self.logout()
