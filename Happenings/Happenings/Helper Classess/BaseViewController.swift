...
class BaseViewController: UIViewController {
    ...
    func setUpNavigation(vc:UIViewController, isBackButton:Bool = false, btnTitle:String = "", navigationTitle:String, action: Selector, isRightBtn:Bool = false, actionRight: Selector = #selector(onClickRightBtn(_:)), btnRightImg:UIImage = #imageLiteral(resourceName: "search") ){
        if isBackButton{
            navigationBar.btnMenu.setImage(#imageLiteral(resourceName: "Back"), for: UIControl.State())
        }
        else{
            navigationBar.btnMenu.setImage(#imageLiteral(resourceName: "menu"), for: UIControl.State())
        }
