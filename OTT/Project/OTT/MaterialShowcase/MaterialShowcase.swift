...
public class MaterialShowcase: UIView {
  ...
  let BACKGROUND_ALPHA: CGFloat = 0.8
  ...
  let BACKGROUND_DEFAULT_COLOR = UIColor.black
...
extension MaterialShowcase {
  ...
  private func addBackground() {
    ...
    backgroundView = UIView(frame: CGRect(x: 0, y: 0, width: radius * 4,height: radius * 4))
...
//    backgroundView.asCircle()
  ...
  @objc public func completeShowcase(animated: Bool = true) {
    ...
    if delegate != nil {
        let messageDataDict:[String: String] = ["message": self.primaryText]
        NotificationCenter.default.post(name: NSNotification.Name(rawValue: "ShowNextShowCaseView"), object: nil, userInfo: messageDataDict)
