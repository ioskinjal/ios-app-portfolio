...
class AssetPersistenceManager: NSObject {
    ...
    func restorePersistenceManager() {
        ...
        guard assetDownloadURLSession != nil else { return }
        ...
        assetDownloadURLSession.getAllTasks { tasksArray in
            ...
            for task in tasksArray {
                ...
                let asset = Asset(stream: stream ?? Stream(), urlAsset: urlAsset)
    ...
    func downloadStream(for asset: Asset) {
        ...
        guard let task =
            assetDownloadURLSession.aggregateAssetDownloadTask(with: asset.urlAsset,
                                                               ...
                                                               options:
                                                                [AVAssetDownloadTaskMinimumRequiredMediaBitrateKey: asset.stream.bit_rate]) else { return }
    ...
    func  localAssetForStream(withName name: String) -> Asset? {
        ...
        do {
            if let url = try? URL(resolvingBookmarkData: localFileLocation,
                ...
                asset = Asset(stream: stream ?? Stream(), urlAsset: urlAsset)
...
//                let filePaths = try fileManager.contentsOfDirectoryAtPath("\(documentsUrl)")
            ...
            Log(message: "An error occured deleting the file: \(error)")
        }
    }
...
extension AssetPersistenceManager: AVAssetDownloadDelegate {
        ...
        if let error = error as NSError? {
            ...
            case (NSURLErrorDomain, NSURLErrorCancelled):
                ...
                do {
                    ...
                    Log(message: "An error occured trying to delete the contents on disk for \(asset.stream.name): \(error)")
                }
                
                userInfo[Asset.Keys.downloadState] = Asset.DownloadState.notDownloaded.rawValue
                
            case (NSURLErrorDomain, NSURLErrorUnknown):
                Log(message:"Downloading HLS streams is not supported in the simulator.")
            ...
            default:
                ...
                Log(message: "An unexpected error occured")
            ...
            do {
                ...
                Log(message: "Failed to create bookmarkData for download URL.")
            }
    ...
    func urlSession(_ session: URLSession, aggregateAssetDownloadTask: AVAggregateAssetDownloadTask,
        ...
        var userInfo = [String: Any]()
        if let tempString = willDownloadToUrlMap[aggregateAssetDownloadTask]?.absoluteString, let url = URL(string: tempString),
            let size = self.getHLSFileSize(at: url.path) {
            userInfo["download_size"] = String(size)
        }
        userInfo[Asset.Keys.name] = asset.stream.name
