//
//  DetailsViewController_Old.swift
//  OTT
//
//  Created by Portal on 22/09/22.
...
class DetailsViewController_Old: UIViewController,UITableViewDelegate,UITableViewDataSource,UICollectionViewDataSource,UICollectionViewDelegate, UICollectionViewDelegateFlowLayout,AccontDelegate, PaymentSuccessVCDelegate, DetailsTableViewCellProtocal, CastTableViewCellProtocal, PlayerViewControllerDelegate,DefaultViewControllerDelegate,GCKSessionManagerListener ,GCKRemoteMediaClientListener,GCKRequestDelegate,GCKUIMediaControllerDelegate,GCKUIMiniMediaControlsViewControllerDelegate,ProgramRecordConfirmationPopUpProtocol,ProgramStopRecordConfirmationPopUpUpProtocol,ExpandableLabelDelegate,PartialRenderingViewDelegate,GADBannerViewDelegate,StartWatchPartyViewProtocol,StartWatchPartyConfirmationViewProtocol {
    ...
    override func viewDidLoad() {
        ...
        if appContants.appName == .aastha {
            self.channelInfoView?.backgroundColor = AppTheme.instance.currentTheme.navigationViewBarColor
        } else {
        ...
        if self.contentDetailResponse?.pageButtons.record != nil {
            ...
            self.recordButtonWidthConstraint.constant = 0
        }
...
//        }
        ...
        if appContants.appName == .gac {
            ...
            if let channelTitle = self.contentDetailResponse?.data[0].paneData {
                self.navigationTitleLbl.text = (channelTitle as! Content).title
            }
...
//        self.detailsTableView.reloadData()
    ...
    func navigateToPlayerFromHeaderMenu(buttonElement : Element) {
        ...
        if self.contentType == "network"{
            LocalyticsEvent.tagEventWithAttributes("\(self.navigationTitleLbl.text!)_Details_page", ["Partners":"", "Language":OTTSdk.preferenceManager.selectedLanguages])
        ...
        if self.sectionDetailsObjArr[indexPath.section] is Content {
            if indexPath.row == 0 {
                ...
                if self.contentDetailResponse != nil {
                    ...
                    if  contentDetailsObj != nil && contentDetailsObj?.backgroundImage .isEmpty == false
                    {
                    ...
                    for lblBtnDataRow in labelBtnsDataRows {
                        ...
                        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + .milliseconds(300)) {
                            ...
                            if cell.rentInfoLabel?.text?.count ?? 0 > 0 {
                                ...
                                cell.rentInfoHeightConstraint?.constant = 0
                                cell.buttonsBgViewHeightConstraint?.constant = 140
                            }
                        }
                    }
                    ...
                    cell.buttonsStackViewWidthConstraint.constant = 200
                    ...
                    if appContants.appName == .gac {
                        cell.watchlistButton?.titleLabel?.font = UIFont.ottRegularFont(withSize: 12)
                        cell.watchlistButton?.imageView?.contentMode = .left
                        cell.watchlistButton?.tintColor = AppTheme.instance.currentTheme.cardSubtitleColor
                        cell.watchlistButton?.setTitle("Favourite", for: .normal)
                        ...
                        cell.watchlistButton?.viewCornerDesignWithBorder(AppTheme.instance.currentTheme.cardSubtitleColor)
                    } else {
                ...
                // let rFont = UIFont.ottBoldFont(withSize: 14.0)
                
                
                //                let attributes = [NSAttributedString.Key.font: rFont,
                //                                  NSAttributedString.Key.foregroundColor : UIColor.init(hexString: "d90738")] as [NSAttributedString.Key: Any]
                //                cell.expandableLabel.setLessLinkWith(lessLink: " ...Read Less",attributes: attributes as [NSAttributedString.Key : Any], position: .right)
                ...
                cell.expandableLabel.collapsed = appContants.appName == .gac ? true : false
                cell.expandableLabel.text = self.movieDescriptionText
                cell.backgroundColor = UIColor.clear
                return cell
            }
        }
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        if self.sectionDetailsObjArr[indexPath.section] is Content {
            if indexPath.row == 0 {
                return self.contentCellHeight + self.tvShowCellHeight
            } else {
                self.detailsTableView.rowHeight = UITableView.automaticDimension
...
extension DetailsViewController_Old {
    ...
    func setGradientBackground() {
        ...
        gradientLayer.frame = CGRect(x: 0, y: self.movieBGImageView.frame.size.height/2 + (UIApplication.shared.statusBarView?.frame.height)!, width: self.view.frame.width, height: self.movieBGImageView.frame.height/2)
