...
- (instancetype)init {
    ...
    if (self) {
        ...
        self.storyboard = [UIStoryboard storyboardWithName:@"CC_iPhone" bundle:nil];
    }
    return self;
}
...
- (void)chooseDevice:(id)sender {
    BOOL showPicker = YES;
//     if (appDelegateDefault.isCCCalled == 1) {
...
//    if (self.controller && ![self.controller isKindOfClass:[CastViewController class]]) {
...
//    YuppLog(@"Received notification that device disconnected");
...
//    YuppLog(@"Received notification that app disconnected");
    
    if (error) {
//        YuppLog(@"Application disconnected with error: %@", error);
    }
...
//    YuppLog(@"device found - %@", device.friendlyName);
...
//    YuppLog(@"device went offline - %@", device.friendlyName);
    [[NSNotificationCenter defaultCenter] postNotificationName:@"castScanStatusUpdated" object:self];
    [self updateCastIconButtonStates];
}
...
//    YuppLog(@"Media control channel status changed");
...
//    YuppLog(@"Media control channel metadata changed");
    [[NSNotificationCenter defaultCenter] postNotificationName:@"castMediaStatusChange" object:self];
}
...
//    YuppLog(@"Connecting to device address: %@:%d", device.ipAddress, (unsigned int)device.servicePort);
...
//        if ([_controller isKindOfClass:[CastViewController class]]) {
//            _controller.navigationItem.rightBarButtonItem = _castIconButton;
//        }
//        appDelegateDefault.castBarButtonItem = _castIconButton;
    }
}
